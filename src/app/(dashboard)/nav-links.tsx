'use client'

import { useState, useEffect } from 'react'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { 
  LayoutDashboard,
  FileInput, 
  Database,
  ChevronDown,
  HeartPulse,
  Target,
  TrendingUp,
} from 'lucide-react'
import { cn } from '@/lib/utils'

interface NavLinksProps {
  isCollapsed?: boolean
  onClick?: () => void
}

const dashboardSubItems = [
  {
    name: 'Ringkasan',
    href: '/dashboard',
    icon: HeartPulse,
    exact: true, // only active when exactly /dashboard
  },
  {
    name: 'Metrik Utama',
    href: '/dashboard/target',
    icon: Target,
    exact: false,
  },
  {
    name: 'Ads Performance',
    href: '/dashboard/ads',
    icon: TrendingUp,
    exact: false,
  },
]

export function NavLinks({ isCollapsed, onClick }: NavLinksProps) {
  const pathname = usePathname()

  // Auto-expand dashboard group when on any /dashboard route
  const isDashboardActive = pathname?.startsWith('/dashboard')
  const [dashboardOpen, setDashboardOpen] = useState(isDashboardActive ?? false)

  // Keep open if navigating within dashboard
  useEffect(() => {
    if (isDashboardActive) setDashboardOpen(true)
  }, [isDashboardActive])

  const isSubItemActive = (href: string, exact: boolean) => {
    if (exact) return pathname === href
    return pathname?.startsWith(href)
  }

  return (
    <nav className="flex flex-col gap-1 px-3">
      {/* ── Dashboard group ───────────────────────────────── */}
      <div>
        {/* Parent button */}
        <button
          onClick={() => setDashboardOpen(v => !v)}
          className={cn(
            "w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group relative",
            isDashboardActive
              ? "bg-indigo-50 text-indigo-700"
              : "text-slate-600 hover:bg-slate-200/60 hover:text-slate-900",
            isCollapsed ? "justify-center" : ""
          )}
        >
          <LayoutDashboard className={cn(
            "h-5 w-5 shrink-0",
            isDashboardActive ? "text-indigo-600" : "text-slate-500 group-hover:text-indigo-600"
          )} />

          {!isCollapsed && (
            <>
              <span className="flex-1 text-left truncate animate-in fade-in slide-in-from-left-2 duration-300">
                Dashboard
              </span>
              <ChevronDown className={cn(
                "h-4 w-4 shrink-0 transition-transform duration-200",
                dashboardOpen ? "rotate-180" : ""
              )} />
            </>
          )}

          {/* Collapsed tooltip */}
          {isCollapsed && (
            <div className="absolute left-full ml-4 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
              Dashboard
            </div>
          )}
        </button>

        {/* Sub-items */}
        {(dashboardOpen || isCollapsed) && (
          <div className={cn(
            "flex flex-col gap-0.5 transition-all duration-200",
            isCollapsed ? "mt-1" : "mt-0.5 pl-3"
          )}>
            {dashboardSubItems.map(sub => {
              const active = isSubItemActive(sub.href, sub.exact)
              return (
                <Link
                  key={sub.href}
                  href={sub.href}
                  onClick={onClick}
                  className={cn(
                    "flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 group relative",
                    active
                      ? "bg-indigo-600 text-white shadow-md shadow-indigo-200"
                      : "text-slate-500 hover:bg-slate-200/60 hover:text-slate-900",
                    isCollapsed ? "justify-center" : ""
                  )}
                >
                  <sub.icon className={cn(
                    "h-4 w-4 shrink-0",
                    active ? "text-white" : "text-slate-400 group-hover:text-indigo-600"
                  )} />

                  {!isCollapsed && (
                    <span className="flex-1 truncate animate-in fade-in slide-in-from-left-2 duration-300">
                      {sub.name}
                    </span>
                  )}

                  {/* Collapsed tooltip */}
                  {isCollapsed && (
                    <div className="absolute left-full ml-3 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                      {sub.name}
                    </div>
                  )}
                </Link>
              )
            })}
          </div>
        )}
      </div>

      {/* ── Other top-level links ─────────────────────────── */}
      {[
        { name: 'Pencapaian Harian', href: '/input-harian', icon: FileInput },
        { name: 'Master Data', href: '/master-data', icon: Database },
      ].map(link => {
        const active = pathname?.startsWith(link.href)
        return (
          <Link
            key={link.href}
            href={link.href}
            onClick={onClick}
            className={cn(
              "flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all duration-200 group relative",
              active
                ? "bg-indigo-600 text-white shadow-md shadow-indigo-200"
                : "text-slate-600 hover:bg-slate-200/60 hover:text-slate-900",
              isCollapsed ? "justify-center" : ""
            )}
          >
            <link.icon className={cn(
              "h-5 w-5 shrink-0",
              active ? "text-white" : "text-slate-500 group-hover:text-indigo-600"
            )} />

            {!isCollapsed && (
              <span className="flex-1 truncate animate-in fade-in slide-in-from-left-2 duration-300">
                {link.name}
              </span>
            )}

            {isCollapsed && (
              <div className="absolute left-full ml-4 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                {link.name}
              </div>
            )}
          </Link>
        )
      })}
    </nav>
  )
}
